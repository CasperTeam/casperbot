    ...
    def decorator(func):
        async def wrapper(client, message):
            ...
            if only_if_admin and not await is_admin_or_owner(
                ...
                await edit_or_reply(
                    message, "`shhh! This Command Only Works, If You Are Admin Of The Chat!`"
                )
                return
            if group_only and chat_type != "supergroup":
                await edit_or_reply(message, "`try in group its a super group`")
                return
            ...
            if ignore_errors:
                ...
                try:
                    ...
                    text += "\n\n__You can Forward This to @casperbotsupport, If You Think This is Serious A Error!__"
                    try:
    ...
    def decorator(func):
        async def wrapper(client, message):
            try:
                ...
                text += "\n\n__You can Forward This to @casperbotsupport, If You Think This is A Error!__"
                try:
                    await client.send_message(Config.LOG_GRP, text)
                except BaseException:
                    logging.error(text)
