...
async def chnnlzip(client, message):
    ...
    shutil.make_archive(str(f"{chnnl}_ZippedBysatyaUB"), "zip", dirz)
    await pablo.edit("`Zipped! Uploading Now!`")
    zip_name = f"{chnnl}_ZippedBysatyaUB.zip"
...
async def chnnlpdf(client, message):
    ...
    if not images_path:
        ...
        shutil.rmtree(dirz)
        return
    with open("imagetopdf@satyabot.pdf", "wb") as f:
        f.write(img2pdf.convert(images_path))
    capt = f"**CONVERTED** \n**Total Images :** `{len(images_path)}` \n**Channel / Group :** `{chnnl}`"
    await client.send_document(message.chat.id, "imagetopdf@satyaot.pdf", caption=capt)
    os.remove("imagetopdf@satyabot.pdf")
    shutil.rmtree(dirz)
    await pablo.delete()
